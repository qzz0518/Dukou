<div align="center">
  ...
  <p>也是合规的微信聊天记录批量文本化备份工具：群聊和朋友圈批量导出成按时间排好的 TXT，附原图、原视频和离线 HTML 预览；只用微信自带的转发和导出，不读数据库、不解密、不注入。</p>
...
<p align="center">
  <img src="Resources/Screenshots/backup-preview.png" width="1100" alt="快捷朋友圈转发设置，以及导出 ZIP 里自带的离线 HTML 聊天预览" />
</p>
...
- **快捷微信转发（实验）**：填群名和条数，Dukou 在微信里多选、合并转发，ZIP 粘给当前 App 或存进文件夹；图片和视频可以不带，超过 100 条自动分批，可合并成一个 ZIP，按 ESC 中断。
- **HTML 离线预览**：ZIP 里附一个 `index.html`，解压后在浏览器里按聊天气泡翻阅，可搜索、按日期查看、看图片和音视频。
- **快捷朋友圈转发（实验）**：按条数导出朋友圈，正文按时间线整理成 TXT，图片和视频放进同一个 ZIP；逐条下载较慢，按 ESC 中断。
- **续聊与按群 Prompt**：每个群记住自己的 Prompt；同一个群再次发送时，自动提示只处理上次之后的新消息。
- **Markdown 导出**：快捷微信转发存到文件夹时可选 Markdown，得到一篇笔记加附件的文件夹，直接放进 Obsidian。
- **批量文本化备份**：快捷转发选「存到文件夹」就是一次备份，群聊和朋友圈批量整理成按时间排好的 TXT，图片视频按需附带，按名称、时间范围和条数命名。
...
**部分微信账号上，「快捷微信转发」和「快捷朋友圈转发」读不到微信界面，会提示「微信没有向这个账号开放界面控件」。** 手动多选后从转发菜单送到 Dukou 不受影响。
...
| 当前聊天的名称 | 授权辅助功能后，转发到达时读一次微信窗口上的聊天名，用于命名和按群记住 Prompt；只存在本机 |
