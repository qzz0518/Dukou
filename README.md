...
再往前一步，「快捷微信转发」（实验）按群名和条数把指定群最近的消息自动导出、校验并粘贴给 agent，不用自己去多选：500 条消息全程约 20 秒。
...
- **快捷微信转发（实验）**：填群名和条数，Dukou 通过辅助功能在微信里多选、合并转发，校验导出的 ZIP 后粘贴到你正在用的 App，目标在后台或最小化也送得到。500 条消息全程约 20 秒，超过 100 条自动分批。执行期间角落有一枚状态条报进度、随时可取消，这段时间鼠标和键盘归 Dukou。
