...
| **发送到自定义** | 你自己的应用清单：一个直接发过去，多个才弹一张小卡片让你选 |
...
- **快捷微信转发（实验）**：填群名和条数，Dukou 通过辅助功能在微信里多选、合并转发，校验导出的 ZIP 后粘贴到你正在用的 App。
...
```text
Sources/
├── DukouCore/    收件箱协议、manifest、转发意图、批次状态；App 与扩展共用
